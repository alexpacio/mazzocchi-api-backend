...
pub async fn register_user_handler(
    ...
    let user = sqlx::query_as!(
        User,
        "INSERT INTO users (name,email,password,is_admin,customer_name) VALUES ($1, $2, $3, $4, $5) RETURNING *",
        body.name.to_string(),
...
pub async fn get_orders_handler<B>(
    ...
    let mut count_query =
        Query::new("select COUNT(*) from SRLMAZZ_LANTEK.dbo.VGiacenzaLamiere WHERE Udata1 = @P1");
    count_query.bind(req.extensions().get::<User>().unwrap().customer_name.as_str());
    ...
    let page_size: f32 = match params.size.unwrap_or(20 as u32) {
        0 => 20 as f32,
        _ => params.size.unwrap_or(20 as u32) as f32,
    };

    let total_pages = (total_count.unwrap() as f32 / page_size).ceil() as u32;
    ...
    let mut query = Query::new(
        "select * from SRLMAZZ_LANTEK.dbo.VGiacenzaLamiere
        WHERE Udata1 = @P1
        ORDER BY Codice DESC
        OFFSET @P2 ROWS
        FETCH NEXT @P3 ROWS ONLY",
    );
    query.bind(req.extensions().get::<User>().unwrap().customer_name.as_str());
    query.bind(offset);
